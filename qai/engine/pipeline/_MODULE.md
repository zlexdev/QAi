...
- `PentestContext` (`contracts.py`) — mutable `@dataclass(slots=True)` threaded through
  ...
  it between MCP tool calls (each of which is otherwise stateless). `safe_mode: bool =
  True` gates any `ACTIVE` check stage exactly like `PluginRunner`'s (see
  `qai/engine/plugins/_MODULE.md`) — `qa_pipeline_start`'s `own_target` flows into it.
...
Fixed stage list built by `qai/mcp_server.py::_build_stages` from `plugins=[...]`
(default `["security_headers"]`); ACTIVE stages (`idor`/`auth_bypass`) can be added the
same way once `own_target=True`. `run_scan`/`run_crawl` are NOT refactored onto this
abstraction — they stay parallel, unmodified code paths. Crawl-wide driving
(multi-page) is deferred to a follow-up plan.

`CheckStage.__init__(check, session)` takes the SAME live `CaptureSession` `ReconStage`
uses — needed so an `ACTIVE` stage's `ReplayClient` binds to the recon session, not a
second one.
